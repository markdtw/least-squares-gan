...
class LSGAN():
    ...
    def generator(self, z):
        with tf.variable_scope('gen'):
            with slim.arg_scope([slim.fully_connected, slim.conv2d_transpose], normalizer_fn=slim.batch_norm):
    ...
    def discriminator(self, images, reuse=False):
        with tf.variable_scope('dis', reuse=reuse):
            with slim.arg_scope([slim.conv2d], activation_fn=None, normalizer_fn=slim.batch_norm, stride=2):
                net = slim.conv2d(images, 64, 5, normalizer_fn=None)
                net = lrelu(net)
                net = slim.conv2d(net, 128, 5)
                net = lrelu(net)
                net = slim.conv2d(net, 256, 5)
                net = lrelu(net)
                net = slim.conv2d(net, 512, 5) # shape = (batch, 7, 7, 512)
                net = lrelu(net)
