...
def train(args):
    queue_loader = Queue_loader(args.dataset, args.bsize)
    model = LSGAN()

    # first, generate fake images through G
    generated_op = model.generator(queue_loader.z_ph)
    # second, get real and fake output from D
    d_fake_output_op = tf.reduce_mean(model.discriminator(generated_op, reuse=False))
    d_real_output_op = tf.reduce_mean(model.discriminator(queue_loader.images, reuse=True))
    # third, the least-square loss
    d_loss_op = tf.square(d_real_output_op - 1) * 0.5 + tf.square(d_fake_output_op) * 0.5
    g_loss_op = tf.square(d_fake_output_op - 1) * 0.5
    ...
    d_msum_op = tf.summary.merge([tf.summary.scalar('d real output', d_real_output_op),
                                  tf.summary.scalar('d loss', d_loss_op)])
    g_msum_op = tf.summary.merge([tf.summary.scalar('d fake output', d_fake_output_op),
    ...
    print ('batch size: %d, ep: %d, iter: %d, initial lr: %.3f' % (args.bsize, args.ep, queue_loader.iters, args.lr))
    ...
    while True:
        # update d network
        feed_dict = {queue_loader.z_ph: np.random.uniform(-1., 1., (args.bsize, 1024)).astype(np.float32)}
        d_fake_output, d_real_output, d_loss, _, d_msum, images = sess.run([
            d_fake_output_op, d_real_output_op, d_loss_op, d_train_op, d_msum_op, queue_loader.images], feed_dict=feed_dict)
        
        writer.add_summary(d_msum, (ep-1) * queue_loader.iters + step)
        # update g network
        generated, g_loss, _, g_msum = sess.run([
            generated_op, g_loss_op, g_train_op, g_msum_op], feed_dict=feed_dict)

        writer.add_summary(g_msum, (ep-1) * queue_loader.iters + step)
        if step % 40 == 0:
            print ('epoch: %2d, step: %3d, d_fake: %.3f, d_real: %.3f, d_loss: %.3f, g_loss: %.3f' %
                    (ep, step, d_fake_output, d_real_output, d_loss, g_loss))

        if step % queue_loader.iters == 0:
            ...
            feed_dict = {queue_loader.z_ph: np.random.uniform(-1., 1., (40, 1024)).astype(np.float32)}
